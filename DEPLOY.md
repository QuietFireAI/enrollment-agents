# DEPLOY - Enrollment Swarm (Schools & Childcare) (v0.2, ratified 2026-07-11; extended & ratified 2026-07-18 - owner sign-off)
...
- SIS / enrollment platform (OUT+IN)
- Tour calendar (OUT+IN)
- Tuition billing / payments (OUT+IN)
- E-signature (enrollment agreements) (OUT+IN)
- Immunization registry (where available) (IN)
- Forms platform (OUT+IN)
...
closed at load.
- `config/priority_rules.json` - Ratified waitlist priority + tiebreak rules
- `config/tuition_schedule.json` - Published tuition and fee schedule
- `config/ratio_caps.json` - LICENSED ratios and configured caps - no override
- `config/jurisdiction_table.json` - Immunization windows, licensing clocks
- `config/offer_window_rules.json` - Published seat-offer windows
- `config/message_templates.json` - Approved family templates (no decision content)
- `config/authority_signers.json` - Humans authorized on discount.authority
